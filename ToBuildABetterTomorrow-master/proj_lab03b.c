...
#define REMOTE_FORWAR_THRESHOLD  _IQ(8.0) //so only values above 8 get used for forward speed control. 8.1 is basically translated to 0.1 speed, times scaler ** changed 8.0 to 9.5
...
uint_least8_t bat_actual = 6; // the believed battery level, at fist assume
...
uint_least8_t front_motor_index = 1; // 1 is alway default. If the orrientation sensor breaks, will default to 1.
...
////////////////////////////////////////Energy saving stuff
uint_least16_t HALT_counter = 0;  //countinf till ready2HALT should be made true
...
volatile uint_least32_t TorqueModeCounter = 0; //counting till timeoout in torque mode during fling start
...
_iq target_speed = _IQ(0.0); //in KRPM for eack motor
...
volatile MOTOR_Vars_t gMotorVarsA = MOTOR_Vars_INIT;
volatile MOTOR_Vars_t gMotorVarsB = MOTOR_Vars_INIT;
...
// **************************************************************************
// Patten studio custom PROTOTYPES XXX
...
void batteryPair(); //if batteries are eeqaul, connects both
...
void delayMSinterrupt(uint_least32_t delay_in_ms); //the delay to use whenever interrupt clock timer is enabled. NOTE!! if called when the timer is not running, this fucntion will wait for ever
...
initAll(); //GPIO, SCI, LEDS, System control,
...
////////////////////////////// REMOTE or DEBUGGER control handling and target speed decisions
  if (processUARTbuffer())// looking for speed commands in the buffer, returns true. Looks for other commands too but returns false in those cases
  {
    if (bat_show > 0) //do not allow motor control if batteries are almost dead
    {
      ...
      if (target_speed_temp >= MAXIMUM_SPEED_KRPM) // *dan comment* If remote speed value is > max speed, set target speed to max speed
      {
        target_speed = MAXIMUM_SPEED_KRPM;
      }  //eliminated governer speed control to test acceleration

      if ( target_speed_temp < _IQ(0.0)) // *dan comment* If remote speed value is < min speed value, set target speed to min speed (0)
      {
  ...
  else if ((!silence_state) && (!safety_stop) && (!debug))// no message so we start counting down towards coasting, except if safety stop routine is in effect. In debug mode, do not want to count down to silence
  {
    silence_counter++;
    ...
    {
      coasting = true; // turn of the controller PWM
  ...
  }//END of Receiving and processing remote commands
...
///////////////////////////////////////////////  BOARD ORIENTATION, USER level Mode, and part of POWER handling
 ...
 else if (user_mode == 2) //normal mode, back has 70% to 100% power based on need. Ride feels most like a snowboard
 {
	 if (front_motor_index == 1)
	 {
		 CTRL_setSpd_max_pu(ctrlHandleA, _IQ(1.0));
...
{
	speed_of_faster_motor = gMotorVarsB.Speed_krpm;
}

speed_average_of_motors = _IQdiv(gMotorVarsA.Speed_krpm + gMotorVarsB.Speed_krpm, _IQ(2.0));

if ((_IQabs(gMotorVarsA.Speed_krpm) < _IQ(0.20)) && (_IQabs(gMotorVarsB.Speed_krpm) < _IQ(0.20))) // consider this the wheels have stopped threshold since low speeds are poorly calculated
{
...
////////////////////////////////////////////////////ACCELERATION handling (during normal operation and differently during safety stop)
 ...
 else //safety stop not engaged, the usual case
 {
	 ...
	 if (target_speed_temp < _IQ(0.0)) // target speed is never negative, but temp_target_speed can be. This is used as a signal to put the brakes.
	  ...
	  {
		  target_accel = _IQabs( _IQmpy(speed_average_of_motors - target_speed, _IQ(0.05))) + _IQ(0.1); //nice soft decell. Considers distance from target. Sortof a P controller --- changed from 0.1 to 0.08
	  }
 }
   ...
   if (!fs_mode && !fs_recently_used) // Not in fs_mode now or recently
   {
     if (wheels_are_not_moving) // if we are too slow or arent moving at all, stop
     {
       if (target_speed >= _IQ(0.45)) //but if we want to get going, engage fs
       {
         ...
         fs_power = _IQ(0.15);// this is somewhat arbitrary, .15 has beeen working well. use 0.8 or less if doing lots of desktop testing of FS
         fs_mode = 1;
         fs_recently_used = 1;
       }
...
if (to10thsec_counter > (USER_ISR_FREQ_Hz *0.1)) // 10 times per sec
{
	...
	if (safetyStop_pingPONG_enabled)//Safety Stop Code. XXX
	{
		...
		//conditions below: basically safety stop has not been called yet, bluetooth is not workig, wheels are moving, therefore we need to begin a safety stop. Disabled in debug mode
	...
	}//END of safety stop code



	if (fs_mode || !coasting) // Whenever PWM is active, measure overall current and update battery usage
	{
	...
	if ((!coasting) && (!fs_mode) && (!fs_recently_used)) // normal riding operation, record and adjust back motor performance
	{
		...
		// back motor average is the result of adding the 10% value of the last 10 samples. It is important that we dont add everything and then divide by ten, because of the risk of exceeding the the IQ math range
...
*/
		...
		//////////////////////////////////////////////////Use back motor perfromance data to adjust power
		...
		else if (back_motorI_average > _IQ(7.0) && back_power_limit < _IQmpy(back_motorI_average, _IQ(0.1))) //between 7 and 10 amps, allow for smooth transitions, but only upwards
		{
		...
		//result of the above statements is that we remain at what ever power we reach untill droping all the way back down to < 6amps.
	...
	}// end if !coasting & !fs_mode !fspause, basically end of back motor output manager
...
/////////////////////////////////////////////////////////////COME back to this, enableing this reduce noise at zero speed, but does not allwo propper function after force start
